...
import type { Command, Platform, PlatformColor } from '../../types'

type FilterPlatform = 'Todos' | Platform

const platforms: FilterPlatform[] = ['Todos', 'Linux', 'Windows']

const platformColors: Record<Platform, PlatformColor> = {
...
function CommandList() {
  ...
  const [platform,  setPlatform]  = useState<FilterPlatform>('Todos')
  const [category,  setCategory]  = useState<string>('Todas')
  const [copied,    setCopied]    = useState<number | null>(null)
  const [copyError, setCopyError] = useState<string>('')

  const cliCommands = useMemo<Command[]>(
    () => (commands as Command[]).filter((c) => c.platform === 'Linux' || c.platform === 'Windows'),
    []
  )

  const categories = useMemo<string[]>(() => {
    const cats = Array.from(new Set(cliCommands.map((c) => getCategory(c.title)))).sort()
    return ['Todas', ...cats]
  }, [cliCommands])
  ...
  const filtered = useMemo<Command[]>(() => {
    return cliCommands.filter((c) => {
      ...
      const matchCategory  = category === 'Todas' || getCategory(c.title) === category
      const matchSearch    =
  ...
  }, [cliCommands, platform, category, search])
            ...
            <button
              ...
              style={{
                background: platform === p
                  ? 'linear-gradient(90deg, rgba(59,130,246,0.26), rgba(124,58,237,0.32))'
                  : 'rgba(4,12,30,0.5)',
